...
esp_err_t toggle_led(uint8_t led);
...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    ...
    int formattedStrResult = asprintf(&viewHtmlUpdated, viewHtml,led_r_state ? "ON" : "OFF", led_g_state ? "ON" : "OFF",adc_b_value,adc_b_value);
    ...
    if (strcmp(req->uri, "/?led-1") == 0)
    {
        toggle_led(led1);
    }
    if (strcmp(req->uri, "/?led-4") == 0)
    {
        toggle_led(led4);
    }
...
esp_err_t init_led(void)
{
    ...
    pGPIOConfig.pin_bit_mask = (1ULL << led1) | (1ULL << led4);
